...
interface OperationGeneralFormProps {
    dispositions: Dispositions;
    onDispositionsChange: (value: Dispositions) => void;
    passationMode: PassationMode | undefined;
    onPassationModeChange: (value: PassationMode) => void;
    isInvalidCombination: boolean;
}

export default function OperationGeneralForm({ 
    dispositions, 
    onDispositionsChange,
    passationMode,
    onPassationModeChange,
    isInvalidCombination
}: OperationGeneralFormProps) {
  const [isPluriannuel, setIsPluriannuel] = useState(false);

  return (
    <Card>
      <CardHeader>
        <CardTitle>Informations Générales de l'Opération</CardTitle>
        <CardDescription>
          Renseignez les détails de base de l'opération à planifier.
        </CardDescription>
      </CardHeader>
      <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div>
            <Label>Dispositions appliquées</Label>
             <RadioGroup 
                value={dispositions} 
                onValueChange={onDispositionsChange} 
        ...
        <div>
          ...
          <Select value={passationMode} onValueChange={onPassationModeChange}>
