...
export default function FormulaireDaoAllotissement({ daoId }: { daoId: string | string[] }) {
  ...
  const minGarantie = montantEstimatif ? parseFloat(montantEstimatif) * 0.01 : 0;
  const maxGarantie = montantEstimatif ? parseFloat(montantEstimatif) * 0.015 : 0;
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-4 gap-y-6 py-4">
              ...
              <div className="space-y-2">
                ...
                <Input 
                  ...
                  value={garantieOffre || ''}
                  onChange={(e) => setGarantieOffre(e.target.value)}
                />
                 {parseFloat(montantEstimatif || '0') > 0 && (
